...
public class AllBookingCounselorListtpl extends BaseTpl<MemberOrderCounselorPageBean.Lists> implements View.OnClickListener{
    ...
    private ImageView iv_stageChange;
    ...
    public AllBookingCounselorListtpl(Context context, AttributeSet attrs) {
        super(context, attrs);
    }
    public AllBookingCounselorListtpl(Context context) {
        super(context);
    }
    @Override
    protected void initView() {
        ...
        iv_stageChange= findView(R.id.iv_stateChange);
        iv_stageChange.setOnClickListener(this);
    ...
    @Override
    public void setBean(MemberOrderCounselorPageBean.Lists bean, int position) {
        if(bean!=null){
            ...
            tv_createTime.setText("订单号  "+bean.getNumber());
            if("1".equals(bean.getStatus())){
                tv_intention.setTextColor(getResources().getColor(R.color.text_bg90));
                tv_intention.setBackgroundColor(getResources().getColor(R.color.alpha));
                findView(R.id.ll_statechange).setVisibility(GONE);
                if ("1".equals(bean.getStage())) {
                    tv_intention.setText("预约已取消");
                }
                if ("2".equals(bean.getStage())) {
                    tv_intention.setText("已预约(已结束)");
                }
                if ("3".equals(bean.getStage())) {
                    tv_intention.setText("已考察(已结束)");
                }
                if ("4".equals(bean.getStage())) {
                    tv_intention.setText("意向同意(已结束)");
                }
                if (Integer.valueOf(bean.getStage()).intValue()>=5) {
                    tv_intention.setText("已签约(已结束)");
                }
            }else {
                tv_intention.setTextColor(0xFFE56032);
                tv_intention.setBackgroundColor(0xFFFFF3ED);
                findView(R.id.ll_statechange).setVisibility(VISIBLE);
                if ("1".equals(bean.getStage())) {
            ...
            if("3".equals(bean.getCarrierType())){
                tv_type.setText("土地");
                tv_saleType.setVisibility(GONE);
                tv_price.setText(bean.getProperty());
                tv_area.setText("土地面积  "+ FDDataUtils.addComma(bean.getLandArea()) + "m²");
            }
            ...
            if ("4".equals(bean.getCarrierType())) {
                ...
                if ("2".equals(bean.getSaleRental())) {
                    ...
                    if ("0".equals(bean.getPriceRent())) {
                        ...
                        tv_price.setText(bean.getPriceSell() + "元/m² 起");
                    }
                }
            ...
            if ("6".equals(bean.getCarrierType())) {
                ...
                tv_area.setText("待租售面积  " + FDDataUtils.addComma(bean.getBuildingArea()) + "m²");
                tv_price.setText(bean.getFloor() + "层");
            }
            ...
            tv_position.setText(bean.getContactLocat());
                                    ...
                                    public void onClick(View view) {
                                        ...
                                        AppUtil.getPpApiClient(ac).memberOrderCounselorUpdateStage(orderid, ac.user_id, new ApiCallback() {
                                            ...
                                            public void onApiSuccess(NetResult res, String tag) {
                                                if (res.isOK()) {
                                                    UIHelper.t(_activity,res.message);
                                                    UpdateStageBean bean= (UpdateStageBean) res;
