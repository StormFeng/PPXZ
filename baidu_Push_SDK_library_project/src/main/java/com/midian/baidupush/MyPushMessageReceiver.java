...
//			MessageTool.getMessageTool(context).saveMessage(mDeviceMessage);//入本地库
     ...
     *                       consult-record：咨询记录页
     *                       consult-comment：咨询点评页
     *                       order-detail：订单详细页
     *                       message-detail：消息详细页
     */
    private void showNotification(Context context, DeviceMessage mDeviceMessage) {
        ...
        } else if ("consult-record".equals(mDeviceMessage.getPage())) {//咨询记录页
            Log.d("wqf","questionId::"+questionid);
            intent.putExtra("questionId",questionid);
            intent.setClassName("com.midian.ppaddress", "com.midian.ppaddress.ui.company.AnswerQuestionActivity");//咨询记录
        }

		/*else if ("consult-record".equals(mDeviceMessage.getPage())) {//咨询记录结束跳到评价页
            intent.putExtra("questionId",questionid);
			if ("0".equals(user_type)) {
				intent.setClassName("com.midian.ppaddress","com.midian.ppaddress.ui.personal.MyCommentActivity");//普通用户点评
			} else if ("2".equals(user_type)) {
				intent.setClassName("com.midian.ppaddress","com.midian.ppaddress.ui.personal.ConsultantCommentActivity");//物业顾问点评
			} else if ("4".equals(user_type)) {
				intent.setClassName("com.midian.ppaddress","com.midian.ppaddress.ui.personal.CommentToServiceTalentActivity");//服务 达人点评
			}
		} */

        else if ("consult-comment".equals(mDeviceMessage.getPage())) {
            intent.putExtra("questionid", questionid);
            intent.setClassName("com.midian.ppaddress", "com.midian.ppaddress.ui.company.EvaluateActivity");//评价服务大人
        } else if ("message-detail".equals(mDeviceMessage.getPage())) {
            intent.putExtra("messageid", messageid);
            intent.setClassName("com.midian.ppaddress", "com.midian.ppaddress.ui.company.SystemNewsActivity");//消息详细页
        }
