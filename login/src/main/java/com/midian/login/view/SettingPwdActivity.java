    ...
    public void onApiSuccess(NetResult res, String tag) {
        ...
        if (res.isOK()) {
            if ("authorizeAuthRegister".equals(tag)) {//第三方账号注册
                ...
                AuthRegisterBean item = (AuthRegisterBean) res;
                //保存用户登陆信息 依次为：用户id、手机、性别、用户名、Token、昵称、头像、登陆角色类型Roletype
                ac.saveUserInfo(item.getData().getMemberid(), phone, null, thirdname, item.getData().getAccessToken(), thirdname
                        , thirdhead, item.getData().getRoletype(), item.getData().getRoletype());
