...
public class LoginActivity extends BaseActivity {
    ...
    private void initTitle() {
        ...
        try {
            topbar.setLeftImageButton(R.drawable.icon_back, UIHelper.finish(_activity)).setTitle(R.string.login).setMode(BaseLibTopbarView.MODE_1);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    UMengLoginUtil.DataListener mDataListener = new UMengLoginUtil.DataListener() {
        ...
        @Override
        public void onSuccess(SHARE_MEDIA platform, String uid, String nickName, String headurl) {
    ...
    private void thirdLogin(String uid, String nickName, String headurl, String tag) {
        this.uid = uid;
        this.thirdname = nickName;
        this.thirdhead = headurl;
        this.authType = tag;
        ac.api.getApiClient(LoginApiClient.class).authLogin(ac.device_token, uid, nickName, headurl, tag, this);
    ...
    public void onApiSuccess(NetResult res, String tag) {
        ...
        if (res.isOK()) {
            ...
            if (tag.equals("login")) {//登录成功回调
                UIHelper.t(_activity, res.message);//登陆成功
            ...
            if (res.errorcode.equals("account_not_auth")) {//account_not_auth 第三方登录返回没有授权标识
                Log.d("onApiSuccess333", "onApiSuccess: 第三方登录错误码=" + res.errorcode + "thirdname=" + thirdname);
